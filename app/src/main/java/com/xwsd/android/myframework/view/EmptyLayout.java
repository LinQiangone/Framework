...
import com.xwsd.android.myframework.utils.NetWorkUtils;


/**
 * 空数据界面提示：用来提示数据加载中，出现的各种问题
 * 如：加载中··；加载失败；网络错误；数据为空
 */
...
//                 img.setBackgroundDrawable(SkinsUtil.getDrawable(context,"pagefailed_bg"));
                if (NetWorkUtils.isNetworkConnected(context)) {
                    tv.setText(R.string.load_error_click_to_refresh);
                    img.setBackgroundResource(R.mipmap.page_failed_bg);
                } else {
                    tv.setText(R.string.network_error_click_to_refresh);
                    img.setBackgroundResource(R.mipmap.no_wifi);
                }
            ...
            case NODATA:
                ...
                img.setBackgroundResource(R.mipmap.no_data);
                img.setVisibility(View.VISIBLE);
                animProgress.setVisibility(View.GONE);
                setTvNoDataContent();
                clickEnable = true;
